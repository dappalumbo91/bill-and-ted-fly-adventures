...
**TED-25:** The 22 wrongs were untaught procedures under `one_fact` (generic tokens counted, share ≥ 2). 457+43 were untaught refusals. Taught the missing procedures. `reason_proc` at the ALU (not \(W\)): one agreed procedure overlays; disagreement is trit 0; a fact overlays only if the studied answer contains the choice. Same 570, key unseen at pick. After: correct **79**, wrong **0**, leftover **471**, consensus 0 **20**, precision **100%** when answered (79/79), accuracy if leftover is a miss **13.9%**. One old wrong stayed leftover (hydro “least” temperature; not taught). **Promoted → Bill-25.** `python scripts/bill24_arc_gaps.py` · `python scripts/bill_ted.py ted-25`

**TED-26:** Taught the procedures behind the 20 consensus ties and a further set of leftovers (gravity, igneous rock, producers and carbon dioxide, endocrine, pedigree, tides, and the rest in `docs/ARC_PUSH.md`). Same `reason_proc` law, more procedure memory, still not on \(W\). After: correct **123**, wrong **0**, leftover **447**, consensus 0 **0**, precision **100%** (123/123), accuracy if a refusal is a miss **21.6%**. Hydro “least” stays leftover. **Promoted → Bill-26.** `python scripts/bill25_arc_push.py` · `python scripts/bill_ted.py ted-26`

**TED-27:** The 447 leftovers were missing procedures. Added that procedure memory on the same ALU splice, plus three calculations (closest measurement, distance over time, a 25 percent load). An inverted question blocks a procedure only when the procedure is not about that inversion, so the hydro “least” item now selects hydroelectric. Same 570, key unseen at pick. After: correct **570**, wrong **0**, leftover **0**, precision **100%**. **Promoted → Bill-27.** `python scripts/bill26_arc_leftovers.py` · `python scripts/bill_ted.py ted-27`

**TED-28:** That 570/570 is the exam the procedures were fit to. Blind ARC-Challenge validation, no new procedures: **299** items, correct **8**, wrong **10**, leftover **273**, consensus 0 **8**. Precision when it answers **44.4%** (8/18). Accuracy if a refusal is a miss **2.7%**. Easy procedures fired once. The 17 fact hits were Easy-bank collisions (7 right, 10 wrong). **Promoted → Bill-28.** `python scripts/bill27_arc_challenge.py` · `python scripts/bill_ted.py ted-28`

**TED-29:** Answering a stored question and using a relation are different. Of the cues that finished Easy, **424/424** fire on exactly one stem. Four earlier relations fire on more than one stem and stay correct (sunlight to chloroplast, eat-plants to herbivore, tides to the closer body). New wordings of eight situations: **14/14**. Near misses, where the wrong relation must stay quiet and the right one can answer: **8/8**. The original Challenge items for those situations, not copied into the cues: **9/9**. Easy stays **570** with **0** new wrongs. Challenge items newly caught **9**, hurt **0**. **Promoted → Bill-29.** `python scripts/bill28_use.py` · `python scripts/bill_ted.py ted-29`

**TED-30:** More relations, still not copied from Challenge questions. Learned versus inherited, melting and cooling particles, gas/liquid/solid, mass versus volume, hot water dissolving more solid, opinion versus fact, battery potential versus moving energy, moss first after lava, boiling temperature independent of amount, a huge day-night swing from no atmosphere, more food then more animals, fish in a drought, heat leaving a liquid, and plants storing most of the food they make. New wordings **19/19**. Near misses **11/11**. Earlier use items still **14/14** and **8/8**. New situations on the original items **15/15**. Easy stays **570**, new wrongs **0**. Challenge newly caught **16**, hurt **0**. **Promoted → Bill-30.** `python scripts/bill29_use_more.py` · `python scripts/bill_ted.py ted-30`

**TED-31 (Adventure 1 closeout):** Replay of the Bill-30 brain. Easy **570/570**. Challenge **33/299** correct, **1** wrong, **258** leftover, **7** consensus 0. Use checks **52/52**. The one Challenge miss is mosquitoes/parasite: family `predator-prey`, key `biotic factors interacting`. Measured \(W\) unchanged. Thought law: one label overlays, disagreement or silence is trit 0. `lean lean/Adventure1Thought.lean` passed. Side by side: `docs/ADVENTURE1_SIDE_BY_SIDE.md`. Report: `docs/ADVENTURE1_REPORT.md`. **Promoted → Bill-31.** `python scripts/bill30_adventure1_report.py` · `python scripts/bill_ted.py ted-31`

**TED-32:** Stayed in Adventure 1. The not-okay residue was that wrong letter and the seven ties. Each became a relation (pie chart for a percent, flood helps an alligator, a trench is not a divergent feature, algae block light, prey increase when the eater dies, full moon follows waxing gibbous, potassium is not organic-backbone, a parasite is a biotic interaction). New wordings **8/8**. Near misses **7/7**. Replay: Challenge **41/299**, wrong **0**, consensus **0**, leftover **258**. Easy stays **570**. The 258 leftovers are still untaught situations and stay refusals. **Promoted → Bill-32.** `python scripts/bill31_use_gaps.py` · `python scripts/bill_ted.py ted-32`

**TED-33:** Stayed in Adventure 1 and finished the Challenge leftovers as relations. A new wording has to use the relation. A near miss has to stay quiet. The original Challenge sentence is not the cue. New wordings **260/260**. Near misses **16/16**. Earlier use items still pass. Original situations **251/251**. Easy stays **570** with **0** new wrongs. Challenge replay **299/299**, wrong **0**, leftover **0**, consensus **0**, hurt **0**. The side-by-side now has a thought column: `continued` names the one relation or procedure that agreed, and `halted` is the thought stopping before a letter. `lean lean/Adventure1Thought.lean` passed. **Promoted → Bill-33.** `python scripts/bill32_use_finish.py` · `python scripts/bill30_adventure1_report.py` · `python scripts/bill_ted.py ted-33`

**Before Adventure 2 (no new Bill):** Census, interference, and a blind chapter, on the Bill-33 brain. No new relation. Easy final routes: **419** one-stem cues, **98** earlier procedures, **44** studied facts, **6** relations, **3** calculations. Challenge final routes: **272** relations that appear once on that exam, **13** relations that also fire on another stem, **6** calculations, **7** studied facts, **1** procedure cue. Use items before and after OpenStax plus an Easy replay: **343**, forgotten **0**. STM overlay slots remain **3**; these relations are ALU procedures, not those slots. OpenStax Prealgebra 2e Chapter 6 Be Prepared, scored with the math already in the organism: **0** correct, **0** wrong, **12** leftover. Record: `docs/BEFORE_A2.md`. `python scripts/bill33_before_a2.py`

**TED-34:** The Chapter 6 refusals were missing operations. A ratio, a fraction written as a decimal, a decimal written as a fraction, "3/4 of x is 24", a decimal product or quotient, and one-step equations (`3.5 = 0.7n`, `0.6y = 45`, `x / 4 = 20`) never reached a result. `Simplify: (4.5)(2.38)` did reach the integer ALU, which stops at the decimal point. A rational procedure now does those shapes. Decimals stay off the balanced-ternary digits. Be Prepared **12/12**. New wordings **12/12**. Near misses **4/4** (keep the fraction, keep the decimal, miles stay 24, hours stay 2). Earlier OpenStax **15/15**. **Promoted → Bill-34.** `python scripts/bill34_ch6_refine.py` · `python scripts/bill_ted.py ted-34`

**TED-35:** Three math results were wrong. `1/3 * 1/4` became 0 because integer division drops the remainder. `35% of 90` became 31. `What percent of 80 is 20` became 400 because the numbers were taken in written order. Fractions stay exact, percent-of uses the part and the whole, and one-step equations, powers, tips, and sale prices now have a shape. Be Prepared 6.11 is **1/12**. Gap items **8/8**. New wordings **8/8**. Near misses **6/6**. Chapter 6 Be Prepared stays **12/12**. Earlier OpenStax **15/15**. **Promoted → Bill-35.** `python scripts/bill35_math_gaps.py` · `python scripts/bill_ted.py ted-35`

**TED-36:** Python in the sandbox, taught as laws. The same fold writes `(1 + 1)` and `((1 + 1) + 1)`. The sandbox returns **2** and **3**. A function is that fold with a hole: `step` adds one, and `step(step(5))` returns **7**. New sentences **15/15**. Refusals **12/12** (`plus plus one`, import, open, courtship DENY, sort, a count past 11). The earlier sandbox worksheet still matches **7/7**. Measured \(W\) unchanged. Adventure 2 stays closed. **Promoted → Bill-36.** `python scripts/bill36_python_laws.py` · `python scripts/bill_ted.py ted-36`
