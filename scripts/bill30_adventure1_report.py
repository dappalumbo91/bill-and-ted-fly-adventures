...
from bill32_use_finish import BANK as BANK33, latest_pick, thought_of  # noqa: E402
...
def trace_arc(exam: str, frame, facts) -> list[dict]:
    ...
    for _, row in frame.iterrows():
        ...
        use_lab, use_law = latest_pick(stem, pairs)
        base_lab, base_how, _ = pick_v3(stem, pairs, facts, PROCS_ALL)
        thought = thought_of(stem, pairs, facts)
        if use_law == "conflict":
...
def trace_bank(exam: str, bank: list[dict], facts) -> list[dict]:
    rows = []
    for item in bank:
        pairs = item["choices"]
        texts = dict(pairs)
...
def md_table(rows: list[dict]) -> str:
    ...
    for r in rows:
        lines.append(
            f"| `{cell(r['id'])}` | {cell(r['stem'])} | {cell(r['expected'])} | {cell(r['family'])} | {cell(r['route'])} | {cell(r['thought'])} |"
        )
    return "\n".join(lines)
...
def main() -> int:
    ...
    use_rows = (
        trace_bank("use, first relations", BANK29, facts)
        + trace_bank("use, further relations", BANK30, facts)
        + trace_bank("use, cleaned gaps", BANK32, facts)
        + trace_bank("use, finished leftovers", BANK33, facts)
    )
    ...
    doc["promote_to"] = "Bill-33" if doc["promotes"] else None
...
Question, expected answer, the answer the organism gave, and the thought that continued or halted. Pin AEB2AD. Replay that Bill-33 freezes. Trace sha256 `{trace_sha}`.
...
"""
    ...
    wrong_lines = "\n".join(
        f"| `{cell(r['id'])}` | {cell(r['stem'][:180])} | {cell(r['expected'])} | {cell(r['family'])} | {cell(r['route'])} | {cell(r['thought'])} |"
        for r in chal_wrong
    ) or "| none |  |  |  |  | |"
...
Challenge validation is correct on **{c_chal['correct']}** of **{c_chal['n']}**, wrong on **{c_chal['wrong']}**, refusing **{c_chal['leftover']}**, with **{c_chal['consensus_0']}** ties. A refusal is a halt: no relation matched, and no studied fact contained one choice. Adventure 2 stays closed. Measured \\(W\\) stays frozen. Courtship stays off.
...
"""
    ...
    print(
        f"  TED-33 easy {c_easy['correct']}/{c_easy['n']} "
