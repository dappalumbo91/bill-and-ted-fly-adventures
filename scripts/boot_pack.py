...
A1CHAL = ROOT / "data" / "bill27_arc_challenge.json"
A1USE = ROOT / "data" / "bill28_use.json"
...
def test_identity_phot1_develop() -> None:
    ...
    if bt.get("current_bill") not in (
        ...
        "Bill-5", "Bill-6", "Bill-7", "Bill-8", "Bill-9", "Bill-10", "Bill-11", "Bill-12", "Bill-13", "Bill-14", "Bill-15", "Bill-16", "Bill-17", "Bill-18", "Bill-19", "Bill-20", "Bill-21", "Bill-22", "Bill-23", "Bill-24", "Bill-25", "Bill-26", "Bill-27", "Bill-28", "Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36",
    ):
    ...
    if gx.get("promotes") and "Bill-8" not in (bt.get("bills") or []) and bt.get("current_bill") not in ("Bill-8", "Bill-9"):
        fail("TED-8 promotes but Bill-8 is not on the ledger")
    ok(
        f"bill7_memory_expand TED-8 {gx.get('n_ok')}/{gx.get('n')}  "
        f"STM={gx.get('stm_slots')}  later={len(gx.get('growth_later') or [])}  "
        f"current={bt.get('current_bill')}"
    )
    gr = json.loads(A2GR.read_text(encoding="utf-8"))
    if not gr.get("overall_ok"):
    ...
    if cod.get("promotes") and "Bill-14" not in (bt.get("bills") or []) and bt.get("current_bill") not in ("Bill-14", "Bill-15"):
        fail("TED-14 promotes but Bill-14 is not on the ledger")
    ok(
    ...
    if ing.get("promotes") and "Bill-21" not in (bt.get("bills") or []) and bt.get("current_bill") not in ("Bill-21", "Bill-22"):
        fail("TED-21 promotes but Bill-21 is not on the ledger")
    ok(
    ...
    if arc.get("promotes") and "Bill-24" not in (bt.get("bills") or []) and bt.get("current_bill") not in ("Bill-24", "Bill-25"):
        fail("TED-24 promotes but Bill-24 is not on the ledger")
    ok(
    ...
    if gap.get("promotes") and bt.get("current_bill") not in ("Bill-25", "Bill-26", "Bill-27", "Bill-28", "Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
    ...
    if push.get("promotes") and bt.get("current_bill") not in ("Bill-26", "Bill-27", "Bill-28", "Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
    ...
    if left.get("promotes") and bt.get("current_bill") not in ("Bill-27", "Bill-28", "Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
    ...
    chal = json.loads(A1CHAL.read_text(encoding="utf-8"))
    cc = chal.get("counts") or {}
    if not chal.get("overall_ok") or chal.get("fitted_to_this_exam") is not False:
        fail("bill27_arc_challenge")
    ...
    if chal.get("promotes") and bt.get("current_bill") not in ("Bill-28", "Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
        fail("TED-28 promotes but ledger current_bill is not Bill-28")
    ok(
        f"bill27_arc_challenge TED-28 {cc.get('correct')}/{chal.get('exam_n')}  "
        f"wrong={cc.get('wrong')} leftover={cc.get('leftover')}  "
        f"prec={chal.get('precision_when_answered')}  current={bt.get('current_bill')}"
    )
    use = json.loads(A1USE.read_text(encoding="utf-8"))
    if not use.get("overall_ok") or use.get("fitted_challenge_val_into_cues") is not False:
        fail("bill28_use")
    if int(use.get("easy_new_wrong") or 0) != 0:
        fail("bill28_use easy regression")
    if int(use.get("apply_ok") or 0) != int(use.get("apply_n") or -1):
        fail("bill28_use apply")
    if int(use.get("near_ok") or 0) != int(use.get("near_n") or -1):
        fail("bill28_use near")
    if use.get("promotes") and bt.get("current_bill") not in ("Bill-29", "Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
        fail("TED-29 promotes but ledger current_bill is not Bill-29")
    ok(
        f"bill28_use TED-29 apply {use.get('apply_ok')}/{use.get('apply_n')}  "
        f"near {use.get('near_ok')}/{use.get('near_n')}  "
        f"source {use.get('source_ok')}/{use.get('source_n')}  current={bt.get('current_bill')}"
    )
    use2 = json.loads(A1USE2.read_text(encoding="utf-8"))
    if not use2.get("overall_ok") or int(use2.get("challenge_hurt") or 0) != 0 or int(use2.get("easy_new_wrong") or 0) != 0:
        fail("bill29_use_more")
    if int(use2.get("apply_ok") or 0) != int(use2.get("apply_n") or -1):
        fail("bill29_use_more apply")
    if use2.get("promotes") and bt.get("current_bill") not in ("Bill-30", "Bill-31", "Bill-32", "Bill-33", "Bill-34", "Bill-35", "Bill-36"):
    ...
    ok(
        ...
        f"near {use2.get('near_ok')}/{use2.get('near_n')}  "
        f"caught={use2.get('challenge_newly_caught')} hurt={use2.get('challenge_hurt')}  "
        f"current={bt.get('current_bill')}"
    )
    gaps = json.loads(A1GAPS.read_text(encoding="utf-8"))
    if not gaps.get("overall_ok") or int(gaps.get("challenge_hurt", 1)) != 0 or int((gaps.get("challenge") or {}).get("wrong", 1)) != 0:
        fail("bill31_use_gaps")
    ok(
        f"bill31_use_gaps TED-32 apply {gaps.get('apply_ok')}/{gaps.get('apply_n')}  "
        f"near {gaps.get('near_ok')}/{gaps.get('near_n')}  "
        f"challenge w={(gaps.get('challenge') or {}).get('wrong')}  current={bt.get('current_bill')}"
    )
    stamp = json.loads(A1STAMP.read_text(encoding="utf-8"))
    se, sc, su = stamp.get("easy") or {}, stamp.get("challenge") or {}, stamp.get("use") or {}
    if not stamp.get("lean_ok") or stamp.get("measured_W_changed") is not False:
        fail("adventure1 stamp")
    if int(se.get("correct", 0)) != 570 or int(se.get("wrong", 1)) != 0:
        fail("adventure1 easy replay")
    nsum = int(sc.get("correct", 0)) + int(sc.get("wrong", 0)) + int(sc.get("leftover", 0)) + int(sc.get("consensus_0", 0))
    if nsum != int(sc.get("n", 0)) or int(sc.get("n", 0)) != 299:
        fail("adventure1 challenge replay")
    if int(su.get("wrong", 1)) != 0 or int(su.get("correct", 0)) != int(su.get("n", -1)):
    ...
    ch6 = json.loads((ROOT / "data" / "bill34_ch6.json").read_text(encoding="utf-8"))
    ch6b = ch6.get("blind") or {}
    if (
        not ch6.get("overall_ok")
        or int(ch6b.get("wrong", 1)) != 0
        or int(ch6b.get("leftover", 1)) != 0
        or int(ch6b.get("correct", 0)) != int(ch6b.get("n", -1))
        or int(ch6.get("old_openstax_ok", 0)) != int(ch6.get("old_openstax_n", -1))
    ):
        fail("bill34_ch6")
    ok(
        f"bill34_ch6 TED-34 chapter6 {ch6b.get('correct')}/{ch6b.get('n')}  "
        f"old {ch6.get('old_openstax_ok')}/{ch6.get('old_openstax_n')}  "
        f"current={bt.get('current_bill')}"
    )
    gaps = json.loads((ROOT / "data" / "bill35_math_gaps.json").read_text(encoding="utf-8"))
    gg = gaps.get("gaps") or {}
    if (
        not gaps.get("overall_ok")
        or int(gg.get("wrong", 1)) != 0
        or int(gg.get("leftover", 1)) != 0
        or int(gg.get("correct", 0)) != int(gg.get("n", -1))
        or int(gaps.get("old_openstax_ok", 0)) != int(gaps.get("old_openstax_n", -1))
    ):
        fail("bill35_math_gaps")
    ok(
        f"bill35_math_gaps TED-35 gaps {gg.get('correct')}/{gg.get('n')}  "
        f"old {gaps.get('old_openstax_ok')}/{gaps.get('old_openstax_n')}  "
        f"current={bt.get('current_bill')}"
    )
    py = json.loads((ROOT / "data" / "bill36_python_laws.json").read_text(encoding="utf-8"))
    taught = py.get("taught") or {}
    fresh = py.get("fresh") or {}
    refuse = py.get("refuse") or {}
    if (
        not py.get("overall_ok")
        or py.get("same_law") is not True
        or py.get("sandbox_pairs_ok") is not True
        or py.get("measured_W_changed") is not False
        or int(py.get("free_parameters", 1)) != 0
        or int(taught.get("wrong", 1)) != 0
        or int(taught.get("leftover", 1)) != 0
        or int(taught.get("correct", 0)) != int(taught.get("n", -1))
        or int(fresh.get("wrong", 1)) != 0
        or int(fresh.get("leftover", 1)) != 0
        or int(fresh.get("correct", 0)) != int(fresh.get("n", -1))
        or int(refuse.get("wrong", 1)) != 0
        or int(refuse.get("correct", 0)) != int(refuse.get("n", -1))
    ):
        fail("bill36_python_laws")
    ok(
        f"bill36_python_laws TED-36 taught {taught.get('correct')}/{taught.get('n')}  "
        f"fresh {fresh.get('correct')}/{fresh.get('n')}  "
        f"refuse {refuse.get('correct')}/{refuse.get('n')}  "
        f"current={bt.get('current_bill')}"
    )
    if bt.get("current_bill") != "Bill-36":
        fail("TED-36 promotes but ledger current_bill is not Bill-36")
    ok(
        f"adventure1 replay easy {se.get('correct')}/{se.get('n')}  "
        f"challenge {sc.get('correct')}/{sc.get('n')} w={sc.get('wrong')}  "
        f"use {su.get('correct')}/{su.get('n')}  lean={stamp.get('lean_ok')}  "
        f"current={bt.get('current_bill')}"
    )
