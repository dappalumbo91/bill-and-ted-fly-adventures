...
|--------|--------|----------------|--------|
| OpenStax / BCcampus | CC textbooks XML/PDF | Sequential **lessons** (arithmetic before algebra). Reshape chapter exercises → `{prompt, want, steps}`. | mapped analog; not ingested yet |
